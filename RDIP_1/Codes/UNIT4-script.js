...
{
    corp = document.getElementById("select-corpus").value;
    if(corp=='c1')
    {document.getElementById("corpus-text").innerHTML=corpus1;
    document.getElementById("msg-to-enter-tokens").innerHTML="Enter the number of tokens and types for the above corpus:";
    document.getElementById("iptable").style.display="inline-block";
    
    }
    else if(corp=='c2')
    {document.getElementById("corpus-text").innerHTML=corpus2;
    document.getElementById("msg-to-enter-tokens").innerHTML="Enter the number of tokens and types for the above corpus:";
    document.getElementById("iptable").style.display="inline-block";
    }
    else if(corp=='c3')
    {document.getElementById("corpus-text").innerHTML=corpus3;
    document.getElementById("msg-to-enter-tokens").innerHTML="Enter the number of tokens and types for the above corpus:";
    document.getElementById("iptable").style.display="inline-block";
    }
    else
...
{   //for words count
    ...
    iptoken = document.getElementById('token').value;
    //for unique words count
    fullstr1 = fullstr.toLowerCase();
    fullstr1 = fullstr1.replace(/[".?,]+/g , '');
    fullstr1 = fullstr1.replace(/\s\s+/g, ' ');
    words = fullstr1.split(" ");
    console.log(words)
    uwordlength = new Set(words).size;
    iptypes = document.getElementById('types').value;

    if(iptoken==count && uwordlength==iptypes)
    {
        document.getElementById("token").style.backgroundColor="green";
        document.getElementById("types").style.backgroundColor="green";
        document.getElementById("resultmsg").innerHTML="Correct Answer";
        document.getElementById("resultmsg").style.color="green";
        document.getElementById("resultmsg").innerHTML+="<br><button id='continue'>continue</button>";
        
    }
    else if(iptoken==count && uwordlength!=iptypes)
    {
        document.getElementById("token").style.backgroundColor="green";
    ...
    else if(iptoken!=count && uwordlength==iptypes)
    {
        ...
        console.log("No",count,uwordlength);
        document.getElementById("token").style.backgroundColor="red";
        document.getElementById("types").style.backgroundColor="red";
        document.getElementById("resultmsg").innerHTML="Wrong Answer";
        document.getElementById("resultmsg").style.color="red";
